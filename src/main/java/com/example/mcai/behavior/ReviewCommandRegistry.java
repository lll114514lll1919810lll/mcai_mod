...
public class ReviewCommandRegistry {
    ...
    private SuggestionProvider<CommandSourceStack> approvalIds() {
        return (ctx, builder) -> { for (int id : approvalQueue.getUnresolvedIds()) { var item = approvalQueue.getItem(id); if (item != null) builder.suggest(id, Component.translatable("mcai.review.suggest.item", item.targetPlayerName, item.reason)); else builder.suggest(id); } return builder.buildFuture(); };
    }
    public LiteralArgumentBuilder<CommandSourceStack> createAiReviewCommand() {
        return Commands.literal("aireview")
                ...
                .then(Commands.literal("approve").then(Commands.argument("id", IntegerArgumentType.integer(1)).suggests(approvalIds()).executes(ctx -> { int id = IntegerArgumentType.getInteger(ctx, "id"); return handleApproval(ctx.getSource(), id, true); })))
                .then(Commands.literal("reject").then(Commands.argument("id", IntegerArgumentType.integer(1)).suggests(approvalIds()).executes(ctx -> { int id = IntegerArgumentType.getInteger(ctx, "id"); return handleApproval(ctx.getSource(), id, false); })))
    ...
    private int handleApproval(CommandSourceStack src, int id, boolean approve) {
        ...
        if (approve) { var item = approvalQueue.tryApprove(id); if (item != null) { src.sendSuccess(() -> Component.translatable("mcai.review.approve.done", id, item.targetPlayerName, item.action), true); chatReviewSystem.executeApprovedAction(item); return 1; } else { src.sendFailure(Component.translatable("mcai.review.approve.invalid", id)); return 0; } }
